...
import { Calculator, ArrowLeft, Syringe, BarChart3 } from 'lucide-react'
import Link from 'next/link'
import { Button } from '@/components/ui/button'
import { Card } from '@/components/ui/card'
import { CalculatorForm } from '@/components/calculator/calculator-form'
import { ResultDisplay } from '@/components/calculator/result-display'
import { VaccineStatistics } from '@/components/calculator/vaccine-statistics'
...
export default function CalculatorPage() {
  ...
  const [activeTab, setActiveTab] = useState<'calculator' | 'vaccine-stats'>('calculator')
      ...
      <nav className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            ...
            <Button
              variant={activeTab === 'vaccine-stats' ? 'default' : 'outline'}
              size="sm"
              onClick={() => setActiveTab('vaccine-stats')}
              className="flex items-center gap-2"
            >
          ...
          {activeTab === 'calculator' ? (
            ...
            <div>
              <div className="mb-6">
                ...
                <p className="text-muted-foreground">
                  查看0-18岁完整疫苗接种费用统计和分布
                </p>
              </div>
              
              <VaccineStatistics includeOptional={true} />
            </div>
          )}
        </motion.div>
      </div>
